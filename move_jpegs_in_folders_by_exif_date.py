...
def main():
    ...
    cwdir = os.getcwdu()
    run(cwdir, directory, options.dry_run, options.recursive)

def do_copy(src, dest, dry_run=False):
...
def run(cwdir, directory, dry_run, recursive):
    files = []
    if not recursive:
    ...
    for f in files:
        ...
        #date_directory_name = "%d_%02d_%02d" % (date.year, date.month, date.day)
        date_directory_name = "%d_%02d" % (date.year, date.month)
        ...
        do_copy(f, moved_file_path, dry_run)
